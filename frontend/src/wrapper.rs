...
fn update_pair(from: &String, wrappers: &[TokenWrapperInfo]) -> (Option<TokenInfo>, Option<TokenInfo>){
    ...
    if let Some(tok) = wrappers.iter().find(|t| t.d_token_address == *from || t.c_asset_address == *from) {
...
pub fn Wrapper() -> Element {
    ...
    let mut factory_address = use_signal(|| "".to_string());
        ...
        div { class: "p-8 mt-12 glass w-full max-w-4xl flex flex-col gap-6 items-stretch flex-col-sm",
              ...
              div { class: "panel flex-1",
                    ...
                    div { class: "mt-3 flex items-center justify-between gap-3",
                          select {
                              value: "{from_selected.read().as_ref().map(|t| t.address.clone()).unwrap_or_default()}",
                              class: "{accent_for(&from_selected.read())} flex-1 bg-transparent text-white text-xl font-semibold focus:outline-none",
                              onchange: move |e| {
                                  let address = e.value().to_string();
                                  let (from,to) = update_pair(&address, &wrappers());
                                  from_selected.set(from);
                                  to_selected.set(to);
                              },
                              option { value: "", "Select token A" }
                              { current_wrappers.iter().map(|t| rsx!(
                                  option { value: "{t.d_token_address}", "{t.d_token_symbol}" },
                                  option { value: "{t.c_asset_address}", "{t.c_asset_symbol}" }
                              )) }
                          }
                    }
              ...
              div { class: "panel flex-1",
                    ...
                    div { class: "mt-3 flex items-center justify-between gap-3",
                          span { class: "{accent_for(&to_selected.read())} flex-1 bg-transparent text-white text-xl font-semibold focus:outline-none",
